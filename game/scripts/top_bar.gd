...
# design.md): the reference boards' day counter and icon-tagged counters,
# replacing the bare "tick · alive · rate · trades" label. Pure readout over
# read-only sim queries, refreshed a few times a second; the old HUD label
# stays in the scene (hidden) for the scenario-failed-to-load message.
...
func _refresh() -> void:
	...
	(_cells["day"] as Label).text = "Day %d" % day_of(tick)
	var rate: String = "paused" if main.paused else "%d×" % main.ticks_per_frame
	(_cells["rate"] as Label).text = rate
	(_cells["alive"] as Label).text = str(int(sim.alive_count()))
