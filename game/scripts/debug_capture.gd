...
func _apply_config_overrides() -> void:
	...
	# HUD scale, same 0.5..1.0 range the menu's spin box offers. Exposed here so
	...
	# most likely to fall off the screen. Clamped to the menu's range: above 1.0
	# the logical viewport is smaller than the design size and the fixed-pixel
	# HUD cannot fit (see GameConfig.ui_scale).
