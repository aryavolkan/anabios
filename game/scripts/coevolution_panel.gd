...
const CHARTS := [
	{
		...
		[
			{
				"key": "communicator_frac",
				"label": "Communicator gene",
				"color": Color(0.35, 0.75, 1.0)
			},
			{"key": "mean_skill", "label": "skill meme", "color": Color(1.0, 0.75, 0.25)},
			{
				"key": "mean_social_learning",
				"label": "SocialLearning",
				"color": Color(0.55, 0.9, 0.55, 0.8)
			},
			{
				"key": "mean_individual_learning",
				"label": "IndividualLearning",
				"color": Color(0.9, 0.55, 0.85, 0.8)
			},
		],
	},
	{
	...
	},
	{
		...
		[
			...
			{
				"key": "practice_inbreeding_frac",
				"label": "inbreeding",
				"color": Color(0.95, 0.45, 0.55)
			},
			{
				"key": "practice_child_sacrifice_frac",
				"label": "child sacrifice",
...
func _draw_chart(
	c: Dictionary, cache: Dictionary, n: int, pad: float, plot_w: float, top: float, h: float
) -> void:
	# y-scale.
	var vmax := 1.0
	var vmin := 0.0
...
func _draw_readout(index: int) -> void:
	...
	lines.append(
		(
			"comm=%.2f skill=%.2f"
			% [float(s.get("communicator_frac", 0)), float(s.get("mean_skill", 0))]
		)
	)
	lines.append(
		(
			"div=%.2f match=%.2f"
			% [float(s.get("meme_divergence", 0)), float(s.get("mean_tech_match", 0))]
		)
	)
