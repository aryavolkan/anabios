...
/// The cognitive layer's serialized IQ state — realized `iq` plus the
/// path-dependent `iq_enrich_acc` / `iq_enrich_ticks` juvenile accumulators —
/// must survive a snapshot. A restored world stepped one tick must reproduce the
/// continuous run's state hash; an iq column silently dropped on load (the
/// serde-skip replay footgun) would diverge here.
#[test]
fn cognition_iq_state_survives_save_load_step() {
    const COGNITIVE: &str = include_str!("../../../scenarios/cognitive-coevolution.toml");
    let mut world =
        Scenario::parse_toml(COGNITIVE).expect("parse cognitive scenario").instantiate();
    assert!(world.cognition_enabled, "scenario must enable cognition");
    // Warm up so juveniles develop mid-flight IQ (non-trivial accumulators).
    for _ in 0..150 {
        step(&mut world);
    }
    assert!(
        world.agents.iter_alive().any(|id| world.agents.iq[id as usize] > 0.0),
    ...
    assert_eq!(
        state_hash(&world),
        state_hash(&reloaded),
        "cognitive world diverged after save→load→step (iq / iq_enrich_* not serialized?)",
    );
}

const SCENARIO: &str = include_str!("../../../scenarios/minimal.toml");

/// The anthropogenic arms race must survive a save→load→step round-trip: the
/// per-species `culture_mask` is `#[serde(skip)]` scratch that feeds the
/// threat sense (→ FEAR → movement), so a reload that failed to rebuild it
/// would diverge on the next tick — the serde-skip replay footgun. The mask
/// is refreshed from the serialized `culture_roots` + `species_parents`
/// every tick, which is exactly what this test pins.
#[test]
fn anthro_race_survives_save_load_step() {
    const ANTHRO: &str = include_str!("../../../scenarios/anthro-race.toml");
    let mut world = Scenario::parse_toml(ANTHRO).expect("parse anthro scenario").instantiate();
    assert!(world.anthro_race_enabled, "scenario must enable the arms race");
    assert!(!world.culture_roots.is_empty(), "scenario must tag a culture bearer");
    // Warm up so hunting/kills have populated the hostility records and the
    // threat sense has steered real movement.
    for _ in 0..400 {
        step(&mut world);
    }
    let bytes = save_to_bytes(&world).expect("save");
    ...
    assert_eq!(
        state_hash(&world),
        state_hash(&reloaded),
        "anthro-race world diverged after save→load→step (culture_mask not rebuilt?)",
    );
}
...
fn out_of_africa_earth_survives_save_load_step() {
    ...
    // Under coverage instrumentation each tick is ~5-10x slower; the
    // round-trip semantics don't depend on warm length.
    let warm = if cfg!(coverage) { 60 } else { 300 };
    for _ in 0..warm {
        step(&mut world);
    }
    let bytes = save_to_bytes(&world).expect("save");
