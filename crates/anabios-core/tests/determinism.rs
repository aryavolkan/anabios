...
// Refreshed 2026-07-15: removed the never-read SpeedMax/DietCarnivory
// TraitOverrides knobs. Behavior is identical (neither slot is read; speed/diet
// come from modules) — only those serialized-but-unread genome slots changed
// value in minimal.toml, so the state hash moved.
