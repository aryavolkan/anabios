...
| scale-settlement-t891-fortified-2x.png | `settlement` t891 2× (`ANABIOS_VILLAGE_ERA=2 ANABIOS_VILLAGE_FLAGS=123`) | The same camp forced into its fortified era: timber-frame houses with slate roofs, a log palisade hugging them, a gate with banners to the south, a watchtower, fenced fields outside the wall, the forest standing back from the clearing. |
