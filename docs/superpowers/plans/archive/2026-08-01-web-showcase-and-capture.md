...
- [x] **Step 1: Add `record-web`** near the existing `record` case (`emergence.sh:122`):
...
- [x] **Step 2: Add `capture`** (windowed Godot screenshot) — resolve scenario, `build` the godot crate, launch Godot on `main.tscn` with `ANABIOS_SCENARIO`/`ANABIOS_SEED`/`ANABIOS_SHOT="$out"` env (mirror the `view` case at `:84`, but set `ANABIOS_SHOT`). Note in the help that it needs a real window (not `--rendering-driver dummy`).
- [x] **Step 3: Update the usage/help block** with both new subcommands.
- [x] **Step 4: Smoke.** `scripts/emergence.sh record-web out-of-africa-saga --seed 318` → regenerates `showcase/replay.js`; open `showcase/index.html` and confirm it plays.
- [x] **Step 5: Commit.** `git commit -m "feat(scripts): emergence.sh record-web + capture wrappers"`
...
- [x] **Step 1: Write the workflow.** `on: { workflow_dispatch: {}, push: { tags: ['v*'] } }`. Steps: checkout; build release `anabios-headless`; `./target/release/anabios-headless record --scenario scenarios/out-of-africa-saga.toml --seed 318 --out showcase/replay.js`; upload `showcase/` as a Pages artifact; deploy via `actions/deploy-pages`. This regenerates the data at publish time so the hosted deck is reproducible from source (not the committed 1.9 MB `replay.js`).
- [x] **Step 2: Verify determinism receipt.** The workflow logs the recorded `state_hash` (from `Meta`); document that it must match a local `record` of the same scenario+seed.
- [x] **Step 3: Update `showcase/README.md`** — the hosted URL, the "publish" trigger (tag or manual dispatch), and how to preview locally (`emergence.sh record-web … && open showcase/index.html`).
- [x] **Step 4: Manual test** — trigger `workflow_dispatch`, confirm the Pages URL plays the saga on desktop and mobile widths (the player is responsive; `prefers-reduced-motion` freezes to a still).
- [x] **Step 5: Commit.** `git commit -m "ci(showcase): gated GitHub Pages deploy of the web replay player"`
...
- [x] **Step 1: Author `predator-prey.json`** — a collapse-and-recovery arc: open (letterbox, hud off, speed 16, wide camera, title); `at_event: "Predation"` → cut + highlight + lower-third; `at_event: "PopulationCrash"` → punch zoom + caption; `after` beats for recovery; `{"end": true}`. Use only trigger names from `codex_panel.gd:4-58`.
- [x] **Step 2: Author `dialects.json`** — a meme-sweep arc keying on `DialectFormed`/`MemeSweep`, switching `{"body":"dialect"}` to show dialect hues, camera following a sweeping cluster.
- [x] **Step 3: Author `inventions.json`** — the innovators-vs-traditionalists race keying on `InventionDiscovered`/`InventionAdopted`, toggling the tech panel (`{"panel":{"name":"tech","visible":true}}`).
- [x] **Step 4: Reproducibility check** — each deck must render from a pinned scenario+seed. Document the seed per deck in a leading comment field (the schema tolerates extra keys) or in `showcase/README.md`.
- [x] **Step 5: Smoke each** (needs a window + ffmpeg): `scripts/emergence.sh record predator-prey --seed <s> --max-seconds 60` produces an MP4 with no stalls (every event beat has a `timeout` so it can't hang).
- [x] **Step 6: Commit.** `git commit -m "feat(showcase): predator-prey / dialects / inventions cinematic decks"`
...
- [x] **Step 1: Add ground/body/overlay beats** that showcase the new effects at the right chapters — e.g. `{"ground":"markets"}` during "The Word", camera `punch` on `Market`/`War` events (which now throw embers/trauma), a close `follow` during "The Herd" so footstep trails read. Keep every event trigger with a `timeout`.
- [x] **Step 2: Re-record and eyeball.** `scripts/emergence.sh record out-of-africa-saga --seed 318 --max-seconds 120` — confirm the cinematic uses the effects without manual camera nudging.
- [x] **Step 3: Commit.** `git commit -m "feat(showcase): use tier-2/3 effects in the out-of-africa saga deck"`
