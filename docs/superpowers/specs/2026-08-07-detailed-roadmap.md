...
| # | Item | Detail | Done when |
|---|---|---|---|
| 3.1 | One new emergence subsystem — **done (knowledge ratchet)** | Shipped: `knowledge_enabled` (off by default) + `KnowledgeRatchet` (event 53) + `knowledge-ratchet.toml` + integration tests + goldens; the e1.3 corpus sweep observed `knowledge_ratchet` firing in 8/8 scenario runs | Opt-in flag, off by default; integration test + goldens; new codex event observed in a sweep |
| 3.2 | Determinism & save/load hardening — **done 2026-08-10** | `tests/save_load_roundtrip.rs`: 21 per-flag round-trips (incl. knowledge-ratchet, unilateral-trade, payoff-biased) + grand-theater everything-on — **caught a real bug**: serde-skipped spatial hashes reloaded with default dims on custom-dims worlds, silently diverging next step (fixed: dims re-derived in `load_from_bytes`). `tests/serde_skip_audit.rs` pins all load-time re-derivations; `docs/determinism-contract.md` documents the 3-category skip rule + subsystem checklist. Plan archived | Every subsystem round-trips; determinism suite covers all opt-in flags |
| 3.3 | Perf headroom — **done 2026-08-10 (no cheap win)** | Profiled at 12k agents: `culture_step` ≈ 73% of step in communicator-dense scenarios (the criterion default world has no communicators, so `tick/10000` hides it). New `culture/culture_step/10000_dense` bench isolates it (38 ms). The one provably bit-identical trim (generic-channel-only broadcast accumulation) measured within noise — the cost is the visit count, and parallelization is blocked by in-place serial semantics. Full analysis + next-attempt options (all behavior-altering, need a golden-rehash buy-in): `docs/perf-notes.md` | ≥10% tick improvement at 10k agents, or a written "no cheap win" with profile |
| 3.4 | Docs & onboarding — **done 2026-08-10** | README status list audited against the 25 opt-in flags (codex 21→59 detectors; affect, knowledge, cognition/practices, trade economy, O2b added); `docs/scenarios.md` maps all 42 scenarios → phenomena → flags; `docs/reproduce.md` walkthrough verified verbatim (O1 founder-tag autopsy, trade-freeze sweeps, corpus sweep, viewer); README links ROADMAP + plans index | Newcomer reproduces an emergence run from docs alone |
